...
function convertDateToRelativeDisplayText(time) {
    ...
    if (time != '' && time != null && time != undefined) {
        ...
        return day_diff == 0 && (
            ...
            day_diff > 0 && day_diff < 2 && day_diff + " days ago" ||
            momentDateFormat(time, Config.DateFormat);
    } else {
        return "--";
    }
};
...
function calculateRemainingTime(time) {
    time = time * 1000
    return moment(new Date(time)).from(new Date())
}
