...
const deleteStateAsyncStorage = (item = "appData", optionalData = null) => {
	try {

		let savedState = null
		if (optionalData) {
			savedState = deleteCookie(item, optionalData);
		} else {
			savedState = deleteCookie(item);
		}
		if (typeof window !== 'undefined') {
		...
		console.warn("Error occurred while deleting state. Error: " + error);
		return {};
	}
}
...
const getLanguageLabel = (string) => {
	...
	if (string && string != null && string != "null" && string.trim() != '') {
		const defaultLanguage = (getStateAsyncStorage("defaultLanguage") && Object(getStateAsyncStorage("defaultLanguage")).keys && Object(getStateAsyncStorage("defaultLanguage")).keys.length > 0) ? getStateAsyncStorage("defaultLanguage") : 'Hindi';
		const languageLabelData = getStateAsyncStorage("languageLabelData") || [];
