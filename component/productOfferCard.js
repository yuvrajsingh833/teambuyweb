...
export default function ProductOfferCard({ item, showLogin }) {
                ...
                <div className="product-content">
                    ...
                    <div className="d-flex align-items-center mt-10">
                        ...
                        <div>
                            ...
                            <div className="xs-heading green-text font-12">{Utils.getLanguageLabel("Get on ")}{Utils.convertToPriceFormat(Number(item.gst_amount) + Number(item.price_without_gst) - Number(item.teambuy_offer_price))}{Utils.getLanguageLabel(" with buying")}</div>
                        </div>

                        {!(Number(item.stock) >= Number(item.reserve_stock) && item.stock != 0 && item.reserve_stock != 0) && <div className="ml-auto "> <button type="button" className="cancel-btn gray-tag-small">Out of stock</button></div>}
