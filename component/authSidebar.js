...
export default function AuthSideBar(props) {
            ...
            .then(res => {
                ...
                AuthService.verifyOTP(postParams).then(verifyOTPResponse => {
                    if (verifyOTPResponse.statusCode != 200) {
                        setIsLoading(false)
                        setOTPError(Utils.getLanguageLabel(verifyOTPResponse.message))
                    } else {
                    ...
                    setOTPError(Utils.getLanguageLabel("Please enter a valid OTP"))
                    console.log(`Verify OTP error : ${e}`)
                    setIsLoading(false)
                })
            }).catch(e => {
                setOTPError(Utils.getLanguageLabel("Something went wrong. Please try again"))
                console.log(`Verify OTP error : ${e}`)
                setIsLoading(false)
            })
    }
    ...
    const handleOTPVerification = () => {
        ...
        if (Object.keys(error).length == 0) {
            ...
            setOTPError(Utils.getLanguageLabel(error.otp))
            return false;
        }
    }
    ...
    const doLogin = () => {
        ...
        AuthService.sendOTP({ "mobileNumber": mobileNumber, "userType": 'customer' }).then(response => {
            if (response.statusCode != 200) {
                setIsLoading(false)
                setMobileNumberError(Utils.getLanguageLabel(response.message))
            } else {
                setIsLoading(false)
                setSeconds(30)
    ...
    const handleLogin = () => {
        ...
        if (Object.keys(error).length == 0) {
            ...
            setMobileNumberError(Utils.getLanguageLabel(error.mobileNumber))
            return false;
        }
    }
    ...
    return (
        <>
            ...
            <section className="sidebar-block" id="loginSidebar">
                ...
                {isLoading ? <Loader /> :
                    <div className="plr-27">
                        <div className="md-heading">{Utils.getLanguageLabel("Get your groceries")} <br />{Utils.getLanguageLabel("with Team Buy")}</div>
                        <form className="custom-form mt-30">
                            <div className="form-group pos-rel">
                                <input type="text" maxLength={10} onKeyDown={(event) => event.key === 'Enter' && handleLogin()} className="form-control" placeholder={Utils.getLanguageLabel("Enter your mobile number")} onChange={(event) => { setMobileNumber(event.target.value) }} value={mobileNumber} />
                                <span className="form-icon phone-icon"></span>
                            </div>
                            <span style={{ fontSize: '12px', color: '#D83734' }}>{mobileNumberError}</span>

                            <div className="text-center mt-4">
                                <button className="green-btn" onClick={() => handleLogin()} type="button">{Utils.getLanguageLabel("Get OTP")}</button>
                            </div>
                        </form>
                    </div>
                }
                <div className="xs-heading mt-40 font-12 plr-9">{Utils.getLanguageLabel("By continuing past this page, you agree to our Terms of Service, Cookie Policy, Privacy Policy and Content Policies. All trademarks are properties of their respective owners.")} {new Date().getFullYear()}©{Utils.getLanguageLabel("Teambuy")}. {Utils.getLanguageLabel("All rights reserved.")}</div>
            </section>
            ...
            <section className="sidebar-block" id="mobileVerfication">
                ...
                {isLoading ? <Loader /> :
                    <div className="plr-27">
                        <div className="md-heading">{Utils.getLanguageLabel("Verify your mobile")} <br />
                            <span className="green-text">+91 {mobileNumber}</span></div>
                        <form className="custom-form mt-30">
                            <div className="form-group">
                                <input type="text" onKeyDown={(event) => event.key === 'Enter' && handleOTPVerification()} maxLength={6} className="form-control" placeholder={Utils.getLanguageLabel("Enter the OTP")} onChange={(event) => { setOTP(event.target.value) }} value={OTP} />
                            </div>
                            <span style={{ fontSize: '12px', color: '#D83734' }}>{OTPError}</span>

                            <div className="text-center mt-4">
                                <button onClick={() => handleOTPVerification()} type="button" className="green-btn">{Utils.getLanguageLabel("Verify OTP")}</button>
                            </div>
                        </form>
                        <div className="black-link text-center mt-20">
                            ...
                            }} style={{ cursor: 'pointer' }} className="green-text">{Utils.getLanguageLabel("Resend OTP")}</a><br />
