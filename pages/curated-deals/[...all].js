...
export default function CuratedDealsPage(props) {
    ...
    const getAllProducts = (query) => {
        setIsLoading(true)
        CategoryService.getCuratedDeals({ dealID: query[0] }).then(response => {
            setAllProducts(response.data)
            setIsLoading(false)
        }).catch(e => {
            console.log(`getCuratedDeals error : ${e}`)
            setIsLoading(false)
        })
    }
    ...
    return (
        <>
            <Head>
                <title>Curated Deals - {all && all[1]} | Teambuy</title>
            </Head>
                            ...
                            <div className="d-flex align-items-center">
                                <nav className="custom-breadcrumb global-breadcrumb" aria-label="breadcrumb">
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item">{Utils.getLanguageLabel("CUrated Deals")}</li>
