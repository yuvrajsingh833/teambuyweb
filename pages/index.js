...
export default function Home(props) {
	...
	const [appLinkInput, setAppLinkInput] = useState(null)
	const [appLinkInputError, setAppLinkInputError] = useState(null)
	const [appLinkInputMode, setAppLinkInputMode] = useState('email')
	...
	const handleAppDownloadLink = () => {
		let appLinkInputValidation = Validations.validateField(appLinkInput, { emptyField: `Please enter ${appLinkInputMode}` })
		let error = {};

		if (appLinkInputValidation.error) {
			error['appLinkInput'] = appLinkInputValidation.message
		}

		if (Object.keys(error).length == 0) {
			MasterService.getAppDownloadLink({ email: appLinkInput, mobileNumber: appLinkInput }).then(response => {
				if (response.statusCode != 200) {
					setAppLinkInputError(Utils.getLanguageLabel(response.message))
				} else {
					openSnackbar('Link sent')
				}
			}).catch(e => {
				console.log(`getAppDownloadLink error : ${e}`)
			})
		} else {
			setAppLinkInputError(Utils.getLanguageLabel(error.appLinkInput))
			return false;
		}
	}
						...
						{dashboardData.curatedDeals.map(item => {
							return <div key={`crated_deals_${item.id}`} className="item">
								<div className="curated-deal-box">
									<Link passHref href={{
										pathname: '/curated-deals/[dealID]/[name]',
										query: { dealID: item.id, name: Utils.convertToSlug(item.heading) }
									}}>
										<a>
											<Image
						...
						{dashboardData.dealsOfTheDay.map(item => {
							return <div key={`crated_deals_${item.id}`} className="item">
								<div className="deal-of-the-day-box">
									<Link passHref href={{
										pathname: '/deal-of-the-day/[dealID]/[name]',
										query: { dealID: item.id, name: Utils.convertToSlug(item.heading) }
									}}>
										<a>
											<Image
							...
							<div className="col-md-7">
								...
								<div className="mt-10">
									<div className="custom-radio d-inline-block">
										<input type="radio" id="emailApp" name="app-link-input-mode" onChange={() => setAppLinkInputMode('email')} checked={appLinkInputMode == 'email' ? true : false} />
										<label htmlFor="emailApp">{Utils.getLanguageLabel("Email")}</label>
									</div>
									<div className="custom-radio d-inline-block ml-20">
										<input type="radio" id="phoneApp" name="app-link-input-mode" onChange={() => setAppLinkInputMode('mobile')} checked={appLinkInputMode == 'mobile' ? true : false} />
										<label htmlFor="phoneApp">{Utils.getLanguageLabel("Phone")}</label>
									</div>
								</div>
								<div className="app-link-form mt-10">
									<div className="row">
										<div className="col-md-7">
											<input type="text" className="form-control" placeholder={Utils.getLanguageLabel(`Enter your ${appLinkInputMode}`)} onKeyDown={(event) => event.key === 'Enter' && handleAppDownloadLink()} onChange={(event) => { setAppLinkInput(event.target.value) }} value={appLinkInput} />
											<span style={{ fontSize: '12px', color: '#D83734' }}>{appLinkInputError}</span>
										</div>
										<div className="col-md-5">
											<button className="green-btn" onClick={() => handleAppDownloadLink()}>{Utils.getLanguageLabel("Send Now")}</button>
										</div>
									</div>
								</div>
